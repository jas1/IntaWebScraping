#' transformar_datos_huevo_valores_actuales_func
...
#' @export
#'
#' @examples transformar_datos_huevo_valores_actuales_func (df_procesado)
  ...
  if(nrow(df_procesado)>0){
    ...
    if (nrow(df_ret) > 0) {
      ret <- df_ret |>
        ...
        warning("transformar_datos_huevo_valores_actuales_func: sin datos. para filtro: descripcion=='polla recriada de 16 semanas blanca'")
    }
  }else{
    warning("transformar_datos_huevo_valores_actuales_func: sin datos. df recibido vacio")
  }
  ret
}
